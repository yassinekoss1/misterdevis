...
/**
 * Class Auth_Model_IsolationRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_IsolationRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'ISO' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Isolation();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Isolation'] as $key => $val ) {
