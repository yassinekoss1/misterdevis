...
/**
 * Class Auth_Model_SpaRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_SpaRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'libelle' => 'SPA' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Spa();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Spa'] as $key => $val ) {
