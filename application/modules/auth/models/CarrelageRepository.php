...
/**
 * Class Auth_Model_CarrelageRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_CarrelageRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'CAR' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Carrelage();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Carrelage'] as $key => $val ) {
