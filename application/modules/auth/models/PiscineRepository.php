...
/**
 * Class Auth_Model_PiscineRepository
 *
...
class Auth_Model_PiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'libelle' => 'PISCINE' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Piscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Piscine'] as $key => $val ) {
