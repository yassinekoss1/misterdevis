...
/**
 * Class Auth_Model_CuisineRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_CuisineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'libelle' => 'CUISINE' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Cuisine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Cuisine'] as $key => $val ) {
