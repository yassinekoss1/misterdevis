...
/**
 * Class Auth_Model_RenovationRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_RenovationRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'REN' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Renovation;
    }
    
    
    // Populating qualificaiton object
    foreach (
      $data['Renovation'] as $key => $val
    ) {
