...
/**
 * Class Auth_Model_VerandaRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_VerandaRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'VER' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Veranda();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Veranda'] as $key => $val ) {
