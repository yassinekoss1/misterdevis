...
/**
 * Class Auth_Model_SaunaRepository
 *
 * @author  Youssef Erratbi <yerratbi@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_SaunaRepository extends EntityRepository {
  ...
  public function getNotifications( $justCount = false ) {
    ...
    $query = $this->_em->createQueryBuilder()
                       ...
                       ->setParameter( 'activite', 'SAUNA HAMMAM' )
                       ->orderBy( 'd.date_creation', 'DESC' )
                       ->getQuery();
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'libelle' => 'SAUNA HAMMAM' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_Sauna();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['Sauna'] as $key => $val ) {
