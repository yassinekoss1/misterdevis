...
/**
 * Class Auth_Model_Abri_piscineRepository
 *
 * @author  Aziz Idmansour <aziz.idmansour@gmail.com>
 * @date    23/12/17
 */
class Auth_Model_AbriPiscineRepository extends EntityRepository {
  ...
  public function save( $id_demande = null, $data ) {
    ...
    if ( ! $demande ) {
      ...
      $activite = $this->_em->getRepository( 'Auth_Model_Activite' )->findOneBy( [ 'ref' => 'ABP' ] );
    ...
    if ( ! $qualification ) {
      $qualification = new Auth_Model_AbriPiscine();
    }
    
    
    // Populating qualificaiton object
    foreach ( $data['AbriPiscine'] as $key => $val ) {
