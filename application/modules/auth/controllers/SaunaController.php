...
class Auth_SaunaController extends Zend_Controller_Action {
  ...
  public function editAction() {
    ...
    if (!$demande || $demande->id_activite->libelle !== 'SAUNA HAMMAM') $this->_redirect('/auth/sauna');
      ...
      if ($form->isValid($data)) {
        ...
        if ($qualification) {
          ...
          if (!$notificationSent) {
            ...
            $this->view->ref = 'SUN-' . str_pad($demande->id_demande, 6, '0', STR_PAD_LEFT);
  ...
  public function pdfAction() {
    ...
    $id = $this->getRequest()->getParam('id');
    $qualification = $em->getRepository('Auth_Model_Sauna')->findOneBy(['id_demande' => $id]);
    if (!$qualification) $this->_redirect('/auth/sauna');
